    ...
    if (fileType == 0x4D42)
    {
        ...
        for (i = 0; i < height; i++)
        {
            for (j = 0; j < width; j++)
            {
                ...
                bmpImg->imageData[(height - i - 1) * width + j] = pixVal;
            }
            if (offset != 0)
            {
                for (j = 0; j < offset; j++)
...
PixelBlock GetPixel(BMPImage *img, int location_x, int location_y)
{
    // Initial result pixel block
    PixelBlock result;
    memset(&result, 0, sizeof(result));

    // Directly return 0r0g0b if invalid input
    if (!((location_x < 0) ||
          (location_x >= img->width) ||
          (location_y < 0) ||
          (location_y >= img->height)))
    {
        result = img->imageData[location_y * img->width + location_x];
    }
    return result;
}